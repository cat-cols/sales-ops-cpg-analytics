-- mart/sales/fact_sales_direct_daily.sql
-- Direct sales fact (manufacturer to retailers)
-- New view for manufacturer model
-- Grain: 1 row per sale_date + store_code + sku + channel
-- Source: int.int_direct_dedup

create schema if not exists mart;

create or replace view mart.fact_sales_direct_daily as
with base as (
  select
    sale_date,
    store_code,
