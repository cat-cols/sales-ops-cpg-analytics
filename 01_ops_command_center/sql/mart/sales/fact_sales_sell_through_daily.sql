-- mart/sales/fact_sales_sell_through_daily.sql
-- Sell-through sales fact (wholesaler to retailer)
-- New view for manufacturer model
-- Grain: 1 row per sale_date + store_code + sku + channel
-- Source: int.int_sell_through_dedup

create schema if not exists mart;

create or replace view mart.fact_sales_sell_through_daily as
with base as (
  select
    sale_date,
    store_code,
  ...
  from int.int_sell_through_dedup
